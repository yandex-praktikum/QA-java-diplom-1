...
public class BurgerTest {
    ...
    public void getReceiptTest() {
        ...
        String expected = String.format("(==== %s ====)%n", bun.getName()) + String.format("= %s %s =%n", ingredient.getType().toString().toLowerCase(), ingredient.getName()) + String.format("(==== %s ====)%n", bun.getName()) + String.format("%nPrice: %f%n", burger.getPrice());
    ...
    public void burgerTest() {
        ...
        bun = buns.get(new Random().nextInt(buns.size()));
        List<Ingredient> ingredients = database.availableIngredients();
        ingredient = ingredients.get(new Random().nextInt(ingredients.size()));
        burger.setBuns(bun);
        burger.addIngredient(ingredient);
